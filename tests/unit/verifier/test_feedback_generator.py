...
async def test_heuristic_mode(mock_gate_result_factory, base_verifier_config):
    ...
    verification_result = VerificationResult(passed=False, gates=[gate_result], duration_seconds=1.0)
...
@pytest.mark.asyncio
async def test_rlm_mode_long_log_recursive(mock_gate_result_factory, base_verifier_config, mock_llm_client):
    config = base_verifier_config
    config.feedback_mode = "rlm"
...
@pytest.mark.asyncio
async def test_auto_mode_above_threshold(mock_gate_result_factory, base_verifier_config, mock_llm_client):
    config = base_verifier_config
    config.feedback_mode = "auto"
...
@pytest.mark.asyncio
async def test_rlm_fallback_on_llm_failure(mock_gate_result_factory, base_verifier_config, mock_llm_client):
    config = base_verifier_config
    config.feedback_mode = "rlm"
    ...
    with patch.object(generator, 'compress_log_output', return_value="fallback compressed") as mock_compress:
        feedback = await generator.generate_feedback(verification_result)
        mock_compress.assert_called_once()
