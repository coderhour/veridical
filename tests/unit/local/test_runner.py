...
async def test_run_subprocess_success(config, console):
    ...
    with patch(
        "asyncio.create_subprocess_shell", new_callable=MagicMock
    ) as mock_shell:
        ...
        process_mock.communicate = MagicMock(side_effect=lambda: asyncio.sleep(0, result=(b"hello\n", b"")))
...
async def test_error_context_passing(config, console):
    ...
    with patch("asyncio.create_subprocess_shell", new_callable=MagicMock) as mock_shell:
        process_mock = MagicMock()
        process_mock.communicate = MagicMock(side_effect=lambda: asyncio.sleep(0, result=(b"", b"")))
        process_mock.returncode = 0
