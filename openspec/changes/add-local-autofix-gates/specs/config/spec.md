...
- **AND** `QualityGate` SHALL support optional fields: `warn_only: bool`, `when_files_changed: list[str]`, `exit_code_map: dict[int, str]`, `fix_command: str`
...
#### Scenario: Quality Gate Fix Command Field

- **WHEN** a `QualityGate` is configured with `fix_command: str`
- **THEN** it SHALL be stored as an optional string field (default: `None`)
- **AND** it SHALL be used by default when `autofix_enabled` is true on the Verifier (default: `True`)
- **AND** it SHALL be ignored in Jules mode (`veri run`), which sets `autofix_enabled` to `False`
